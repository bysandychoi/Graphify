...
| `case_ratios.rare_eligible_eqp_count_types` | 예(형태는 아래 참고) | T017 (드문 설비 수 유형) | `eligible_eqp_count_distribution`과 같은 분포를 다시 정의하는 필드가 **아니다.** 특정 `eligible_eqp_count` 값의 건수를 절대 건수로 직접 지정해 "드문 유형"을 만든다: `{"5": {"min_count": 1, "max_count": 5}}`. **T017 구현은 실현 건수를 항상 `min_count` 그대로 쓴다** — `[min_count, max_count]` 범위에서 무작위로 고르지 않는다. `max_count`는 "이 값보다 큰 `min_count`를 실수로 넣지 않았는지" 확인하는 상한 검증 용도이며(`min_count > max_count`면 에러), 실현 건수에는 영향을 주지 않는다. `min_count`는 필수이며, 스펙 객체는 `min_count`/`max_count` 외의 키를 허용하지 않고 빈 객체도 에러다(둘 다 config.md "잘못된 config" 절과 같은 기준). 여기 지정한 값(`5`)은 **`eligible_eqp_count_distribution`에 동시에 지정할 수 없다**(검증 규칙) — 그 값의 건수는 이 필드가 절대 건수로 전담하고, 나머지("드물지 않은") 값들만 `eligible_eqp_count_distribution`이 비율로 분배한다(그 비율은 나머지 값들끼리 합이 1). 유효 lot 수 = 이 필드가 만든 건수 총합 + `eligible_eqp_count_distribution`이 분배하는 나머지. |
...
`rare_eligible_eqp_count_types`가 절대 건수(정확히 `min_count`인 5건,
`max_count`인 8은 검증용 상한일 뿐 실현값이 아니다)로 전담하므로 두
필드에 동시에 넣지 않는다(위 검증
규칙).
...
- `case_ratios.lot_eqp_step_mismatch`가 만드는 불일치의 정확한 형태
  ...
  **참고로 T014 구현(`generator/lot_data.py`)은 이 미결 사항 자체를
  ...
  docstring 참고). `eligible_eqp_count_distribution`이 켜지면(T015,
  `generator/eqp_count_distribution.py`) 목표 개수에 맞춰 그 조합의
  eqp 중 일부만 남기고 나머지를 버리므로, 결과는 "정확히 일치"가
  아니라 그 조합이 제공하는 전체 쌍의 **진부분집합**이 된다 — 부분
  집합은 두 해석 모두의 부분집합이기도 하므로 모순되지 않는다는
  결론 자체는 유지된다. 이는 생성기가 가상 데이터를 만들 때 쓰는
  선택일 뿐, 회사 실제 조인 키가 무엇인지에 대한 이 미결 사항을
  대신 확정하는 것이 아니다.**
